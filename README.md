...
The universal iOS/iPadOS app has passed App Store review and is available from the same multi-platform listing. Version 1.2 is uploaded as macOS build 63 and iOS/iPadOS build 73. The iOS/iPadOS 1.2 record is prepared in App Store Connect with build 73 attached; the Mac 1.2 build is valid in TestFlight while the active Mac 1.1 store submission is still being cleared.
...
The connection screen is code-first but still user-initiated: the 16-digit Mac pairing field is always visible while disconnected. Select a visible or remembered Mac card, enter the code shown in the Mac app (dashes are accepted or inserted automatically), and tap **Connect with Code**. The code authenticates the selected route; it is not itself a network address, so discovery remains passive in the background and never connects just because a Mac was found. After the first successful proof, the device-specific Keychain credential is used automatically.
...
The fallback is a hardware-encoded H.264 HiDPI stream sized from the iPad's native display width (clamped to 1440–2880 pixels, up to 60 fps on the direct route and 30 fps on the nearby fallback) with remote keyboard, trackpad, touch, and Apple Pencil input. Each frame carries its actual encoder cadence so the iPad display clock does not assume the old 30 fps schedule. Direct video keeps a bounded three-frame Network.framework send window rather than serializing capture behind one large frame; this preserves throughput without allowing unbounded latency. The direct profile uses a bounded high-quality bitrate up to 20 Mbps, while the nearby profile uses 8 Mbps and a 1920-pixel cap. Capture buffering is limited to two surfaces; the encoder emits one periodic keyframe per second and immediately requests another when a transport discards a dependency chain. JPEG packets remain supported for compatibility. It mirrors the main display rather than creating a true extra macOS display. Native Sidecar remains the preferred path for a true virtual Retina display, native Apple Pencil behavior, audio, and extended-desktop support.
...
Files can move in either direction over the active encrypted same-Wi-Fi/AWDL or nearby P2P session. Transfers use 128 KB acknowledged chunks, preserve video/input responsiveness, and are limited to 512 MB. The Mac and iPad can queue multiple selected or dropped files, cancel the active transfer, and show live byte progress, transfer rate, and ETA. A completed transfer is exposed only after its SHA-256 digest is verified and its hidden partial file is atomically renamed. Files received by the Mac are saved in the app's private `Application Support/SidecarBridge/Transfers` folder; files received by the iPad can be exported with **Share Received**. An idle transfer now has a 45-second recovery window so slower local links are not canceled prematurely.

Clipboard sync is enabled by default while the encrypted session is connected: copy on either device and no send button is needed. A copied text value uses the bounded control message; files copied in Finder/Files use the verified file-transfer engine and are placed on the receiving device's clipboard after the digest is verified. The **Automatic text and file sync** toggle disables this behavior without disabling the manual **Send Clipboard** and **Receive Clipboard** buttons. Connection baselines, pending local values, and content signatures prevent an old value or a remote write from being echoed back, and a copied file is queued only once even if the pasteboard reports the change more than once. macOS watches its pasteboard while the menu-bar app remains alive, including after the main window is closed. A local change observed during a reconnect or a paused-sync interval is retained and sent when the encrypted session returns or sync is enabled; after iPadOS resumes, the iPad pulls the Mac's latest value only when its own pasteboard did not change. On iPadOS, change notifications plus change-count polling cover active and Picture in Picture sessions; a short iPadOS background grace task protects both a copy made during the app-switch transition and an in-progress file transfer. If iPadOS fully suspends the app, the latest clipboard is reconciled as soon as the app returns because third-party iPadOS apps cannot run an unrestricted clipboard watcher while suspended.

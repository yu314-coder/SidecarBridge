...
The latest uploaded builds are **iOS/iPadOS 1.4 (2)** and **macOS 1.4 (106)**. App Store Connect reports both as `VALID` and `IN_BETA_TESTING`; neither 1.4 binary has been submitted for App Review. The Mac pairing window shows a large QR code and grouped code with Copy Code feedback, while the iPad keeps the manual code path available when discovery is slow. The 1.4 iPad build adds an optional developer-only live frame-interpolation experiment, automatic decoder/keyframe recovery, and a real Picture in Picture background-video path. See the [implementation and testing notes](FRAME_INTERPOLATION_AND_BACKGROUND.md) and [iOS/iPadOS 1.4 (2) release record](releases/1.4-ios2-interpolation-recovery.md).
...
The universal iOS/iPadOS app is available from the same multi-platform listing. The latest TestFlight binaries are iPadOS/iOS **1.4 (2)** and macOS **1.4 (106)**; both are Apple-processed `VALID` builds available to internal testers. The public App Store listing remains unchanged until a separate App Review submission is made. The prior 1.3 update includes refreshed descriptions and screenshots, the in-viewer and connection-screen file-transfer progress banner, iPad File Manager, foreground decoder reset, fresh ScreenCaptureKit capture-source rebuild, and keyframe recovery. The 1.4 experiment adds bounded asynchronous VideoToolbox decoding and automatic fresh-IDR recovery instead of leaving the frame-generation switch in a permanent decoder-error state.

...
The latest submitted builds are **iOS/iPadOS 1.3 (30)** and **macOS 1.3 (105)**. Both are attached to the 1.3 App Store Connect records; review status is recorded below. The Mac pairing window shows a large QR code and grouped code with Copy Code feedback, while the iPad keeps the manual code path available when discovery is slow.

...
The fallback is a hardware-encoded H.264 HiDPI stream with selectable **Adaptive**, **1080p**, or **2K** quality and 30/60/90/120-FPS targets. New installs default to the 120-FPS target; direct local links clamp it to the connected Mac display's highest available refresh mode, while foreground nearby P2P can target up to 120 FPS and 1920 pixels. Bounded queues shed stale work if the route cannot sustain that cadence. When macOS reports warning or critical memory pressure, the Mac temporarily caps capture at 1920 or 1280 pixels respectively, keeps warning pressure at up to 60 FPS, and lowers bitrate instead of forcing a low-cadence recovery loop. Pressure transitions are debounced, and a pressure rebuild does not gate the viewer on a separate refresh signal: the next IDR carries the new dimensions and becomes the decoder boundary. The saved quality and FPS target is restored when pressure clears. The Mac clamps the requested size to the iPad and display capabilities, and each frame carries its actual encoder cadence so the iPad display clock does not assume a fixed rate. Direct video uses a small bounded send window (2/3 in-flight sends at 30–90/120 FPS) and a pending cap one frame above it, while nearby video uses latest-frame unreliable delivery so high-cadence traffic cannot block keyboard or pointer input. The iPad decoder keeps a short 4–6-frame live-edge delivery queue plus a small display queue. If a packet gap or decoder backlog is detected, stale samples are discarded and the Mac is asked for an immediate keyframe rather than waiting for a long recovery interval. Forced IDR requests remain armed until VideoToolbox emits a valid keyframe, so a dropped pressure-period submission cannot leave the viewer showing only the one-second recovery frames. Direct bitrate scales with the selected pixel rate and cadence up to a hard 40 Mbps ceiling, while nearby remains at a bounded 8 Mbps target. Capture buffering uses two IOSurfaces at every cadence; VideoToolbox runs in real-time/no-lookahead mode and emits a periodic recovery keyframe about once per second. The video sender treats `contentProcessed` as local Network.framework progress—not an iPad display acknowledgement—and only enters keyframe recovery after four seconds with no progress. Larger receive reads reduce packet-framing overhead without changing the authenticated payload limit. JPEG packets remain supported for compatibility. It mirrors the main display rather than creating a true extra macOS display. Native Sidecar remains the preferred path for a true virtual Retina display, native Apple Pencil behavior, audio, and extended-desktop support.
